    ...
    <form enctype="multipart/form-data" class="form" action="{{route('admin.settings.update')}}" method="post">
        ...
        @foreach($languages as $language)
            <div class="fv-row mb-7">
                <!--begin::Label-->
                <label class="required fs-6 fw-semibold mb-2">Bilgi  ({{$language->name}})</label>
                <!--end::Label-->
                <!--begin::Input-->
                <textarea type="text" class="form-control form-control-solid tinyMiceEditor" placeholder="" name="info_content_{{$language->code}}_text">{{setting('info_content_'.$language->code."_text")}}</textarea>
                <!--end::Input-->
            </div>
        @endforeach
